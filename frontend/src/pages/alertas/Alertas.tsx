...
export function Alertas() {
  ...
  const handlePrometioPagar = async (alerta: Alerta) => {
    try {
      ...
      showSuccess('Marcado como prometio pagar');
      refetch();
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      showError(msg);
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        title="Cobranzas"
        subtitle={`${deudores} deudores · ${formatCurrency(deudaTotal)} por cobrar · ${total} alertas`}
      />
              ...
              return (
                <div key={a.id || idx} className="bg-slate-800/50 border border-slate-700 rounded-xl p-4 flex flex-col lg:flex-row lg:items-center justify-between gap-3">
                  <div className="flex items-start gap-4 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <p className="text-white font-medium truncate">{a.jugador_nombre || a.titulo || a.nombre || `Alerta #${a.id}`}</p>
                        {a.estado_cobranza && (
                          <Badge variant={estadoVariant(a.estado_cobranza)}>{a.estado_cobranza}</Badge>
                        )}
                      ...
                      <p className="text-sm text-slate-400 mt-1 flex flex-wrap gap-x-3 gap-y-1">
                        ...
                        <span className="font-mono text-[#22C55E] font-bold">Deuda: {formatCurrency(a.deuda || 0)}</span>
                  ...
                  <div className="flex flex-wrap items-center gap-2 flex-shrink-0">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => handleContactado(a)}>Contactado</Button>
                    <Button variant="ghost" size="sm" onClick={() => handlePrometioPagar(a)}>Prometio pagar</Button>
                    <Button size="sm" onClick={handleCobrar}>Cobrar</Button>
                    {waLink ? (
                      ...
                      <Button variant="ghost" size="sm" disabled title="Sin telefono">WhatsApp</Button>
                    )}
                    {a.jugador_id != null && (
                      <Button variant="ghost" size="sm" onClick={() => handleVerJugador(a)}>Ver jugador</Button>
                    )}
                  </div>
                </div>
              );
