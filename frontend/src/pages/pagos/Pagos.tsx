    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          { label: 'Deudores', value: resumen.deudores, color: 'bg-amber-500', textColor: 'text-amber-400', icon: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z' },
          ...
          <div key={label} className="bg-slate-800/50 border border-slate-700 rounded-xl p-4 flex items-center gap-3">
            <div className={`w-9 h-9 rounded-xl ${color} text-white flex items-center justify-center`}>
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d={icon} /></svg>
            </div>
            <div>
      ...
      <details className="bg-slate-800/50 border border-slate-700 rounded-2xl">
        ...
        <div className="p-4 border-t border-slate-700">
          <div className="flex gap-2 mb-3">
            {([['todos', 'Todos'], ['deudores', 'Deudores'], ['pagados', 'Al dia']] as const).map(([val, label]) => (
